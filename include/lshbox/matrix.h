...
public:
    ...
    void load(std::vector<std::pair<std::vector<float>, std::string> > &vec, int _N, int _dim)
    {
        reset(_dim, _N);
    ...
    Matrix(std::vector<std::pair<std::vector<float>, std::string> > &vec, int _N, int _dim): dims(NULL)
    {
        load(vec, _N, _dim);
    }
    ...
    public:
        ...
        typedef const std::vector<T> featureValue;
        typedef T DATATYPE;
